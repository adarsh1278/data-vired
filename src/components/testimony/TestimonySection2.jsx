...
const TestimonySection2 = ({ style }) => {
  ...
  const [totalSlides, setTotalSlides] = useState(Math.ceil(testimonyData.length / 2));
  ...
  const renderSlides = () => (
    <div className="flex flex-nowrap w-full">
      {testimonyData.map((item) => (
        <div
          className={`px-3 md:px-4 ${slidesPerView === 1 ? 'w-full' : 'w-1/2'} shrink-0`}
          key={item.id}
        >
          <div className="bg-white rounded-lg p-4 md:p-8 shadow-md mb-6 relative">
            {/* Large Quote Icon */}
            <div className="absolute top-4 left-4">
              <svg className="w-8 h-8 md:w-12 md:h-12 text-indigo-500" viewBox="0 0 100 100" fill="currentColor">
      ...
      <div className="container mx-auto px-4">
        <DivAnimateX position={-60} className="section-title mb-8 md:mb-16">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-center">
            What Student's Say To Do <br className="hidden md:block" />
            Their Online Course
          </h2>
        </DivAnimateX>
        ...
        <DivAnimateY className="relative">
          <div
            ...
            style={{ transform: `translateX(-${activeSlide * (100 / slidesPerView) * slidesPerView}%)` }}
          >
            {renderSlides()}
          </div>
            ...
            <div className="flex space-x-2 md:space-x-3">
              {Array.from({ length: totalSlides }).map((_, index) => (
                <button
                  ...
                  className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${activeSlide === index ? 'bg-teal-500' : 'bg-gray-300'}`}
